...
格式（2026-09 定稿）：`v?MAJOR.MINOR.PATCH(-prerelease)?`——`sokel-gen check` 拒收其它写法，
分发 registry 还要求必填（缺版本的条目会让「可更新」与安全通告永远沉默）。`v` 前缀可带可不带，
比较时忽略（`v1.0.1` 等于 `1.0.1`）；能解析的版本按 semver 比较，安全通告的范围表达式
（`"<v1.2.0"`）也靠它。解析不了的串走保守路：永不命中范围、只与自身逐字符相等——
所以版本串在所有出现处（manifest、发行 manifest、手动 `SetVersion`）保持一致。
