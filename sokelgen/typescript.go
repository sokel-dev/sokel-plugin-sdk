...
func RenderTS(pkgLabel string, ops []OpIO) (string, error) {
	...
	b.WriteString("// 执行契约（来自 " + pkgLabel + " 的 schema 声明）。改契约请改那边，然后重新生成。\n")
	b.WriteString("//\n// 只含「调用要哪些参数」——UI 怎么让人填是另一回事（widget/分组/拆分控件留在手写 schema 里）。\n")
	b.WriteString("// 前端用它核对手写 schema：契约必填的参数，UI 那侧必须有对应必填字段或显式映射。\n\n")
