...
func RenderCredential(pkg string, sch SchemaRef, fields []Field) (string, error) {
	...
	b.WriteString("//\n// 凭证契约由 schema 声明生成。改凭证请改 schema 声明，然后重新生成。\n\n")
	...
	b.WriteString("// credentialContract 是上报给平台的凭证契约（schema 声明的原样字段）。\n")
	b.WriteString("func credentialContract() []contract.Field {\n\treturn " + renderFields(fields, 1, "contract") + "\n}\n\n")
	b.WriteString("// RegisterCredential 上报本插件的凭证契约（等价于早先的 sokel.WithCredential[Cred]，\n")
	b.WriteString("// 但字段来自 schema 声明，不经反射——enum 候选值、默认值这些 struct tag 写不出来的都在）。\n")
...
func renderCredStruct(fields []Field, schemaPkg string, need *imports) string {
	var b strings.Builder
	b.WriteString("// Cred 是本插件的凭证契约（schema 声明生成）。\n")
	b.WriteString("// 读取用 sokel.CredentialAs[Cred](ctx)，不要 ctx.Credential()[\"裸键\"]——裸键拼错编译不报错。\n")
...
func RenderAuth(pkg string, meta AuthMeta) (string, error) {
	...
	var b strings.Builder
	b.WriteString("// Code generated by sokel-gen. DO NOT EDIT.\n")
	b.WriteString("//\n// 认证方式由 schema 声明生成。改认证请改 schema 声明，然后重新生成。\n\n")
	b.WriteString("package " + pkg + "\n\n")
	b.WriteString("import (\n\t\"github.com/sokel-dev/sokel-plugin-sdk/contract\"\n\t\"github.com/sokel-dev/sokel-plugin-sdk/plugin\"\n)\n\n")
	b.WriteString("// authMeta 是本插件的认证声明（schema 声明的原样内容）。\n")
	...
	b.WriteString("\t}\n}\n\n")
	b.WriteString("// RegisterAuth 声明凭证的获取方式并接住实现。\n")
	b.WriteString("// 参数表由声明的步骤决定——少给一个是编译错，不是运行时才发现面板卡住。\n")
