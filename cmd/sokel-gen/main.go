...
func dispatch(args []string) error {
	...
	case "generate", "check":
		...
		return generate(dirs, *schema, cmd == "check")
	case "export":
...
用法：
  ...
  sokel-gen generate [目录...]     生成；给的目录下有多个插件时自动全扫
  sokel-gen check [目录...]        只校验生成物是否最新，不写文件（CI 用）
  sokel-gen export <格式> [目录]   导出契约：json / ts / python
  sokel-gen migrate [目录]         从旧 struct+tag 反向生成 schema 声明
...
// 比修一个跑一轮快得多。
func generate(dirs []string, schemaSub string, check bool) error {
	var plugins []string
	for _, d := range dirs {
		...
		if len(found) == 0 {
			return fmt.Errorf("%s 下没找到插件（判据：目录里有 %s/ 子目录）", d, schemaSub)
		}
	...
	for _, p := range plugins {
		if err := generateOne(p, schemaSub, check, len(plugins) > 1); err != nil {
			if !check {
...
// discover：找出 root 下的插件。判据是「目录里有 schema/ 子目录」。
// 找到即不再往下钻——插件内部不会再嵌插件，继续走只会撞进它自己的子包。
func discover(root, schemaSub string) ([]string, error) {
	if fi, err := os.Stat(filepath.Join(root, schemaSub)); err == nil && fi.IsDir() {
	...
	err := filepath.WalkDir(root, func(path string, d os.DirEntry, err error) error {
		...
		if fi, serr := os.Stat(filepath.Join(path, schemaSub)); serr == nil && fi.IsDir() {
...
func runExport(args []string) error {
	...
	case "json", "ts", "python":
	default:
		return fmt.Errorf("未知格式 %q（可选 json / ts / python）", format)
	}
